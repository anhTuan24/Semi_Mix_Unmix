...
from mum.data.dataset_mapper import DatasetMapperTwoCropSeparate
from mum.engine.hooks import LossEvalHook
...
class BaselineTrainer(DefaultTrainer):
    ...
    def build_train_loader(cls, cfg):
        if cfg.SEMISUPNET.Suponly:
            return build_detection_sup_train_loader(cfg, mapper=None)
        else:
    ...
    def run_step_full_semisup_mix_unmix(self):
        ...
        if self.iter < self.cfg.SEMISUPNET.BURN_UP_STEP:
            ...
            if self.iter == self.cfg.SEMISUPNET.BURN_UP_STEP:
                ...
                if self.iter < self.semi_args.DYNAMIC_EMA:
                    keep_rate = (self.semi_args.EMA_KEEP_RATE-0.5)/self.semi_args.DYNAMIC_EMA * self.iter + 0.5
                else:
                    keep_rate =self.cfg.SEMISUPNET.EMA_KEEP_RATE
                self._update_teacher_model(
                    keep_rate=keep_rate)
